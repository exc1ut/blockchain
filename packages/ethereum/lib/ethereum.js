  ...
  if (cachedInstances.hasOwnProperty(key)) return cachedInstances[key];
    ...
    const rpcUrl =
      network.host === "ethnodes.get-scatter.com"
        ? "https://twilight-shy-surf.quiknode.pro/1bebe371e965847a5eb3fb6fe73b7c0f782ba991/"
        : network.fullhost();
...
export default class ETH extends Plugin {
  ...
  bufferToHexPublicKeyOrAddress(buffer) {
    return ethUtil.addHexPrefix(
      ethUtil.publicToAddress(ethUtil.importPublic(buffer)).toString("hex")
    );
  }
  ...
  async signer(
    ...
    if (!privateKey)
      privateKey = await KeyPairService.publicToPrivate(publicKey);
    ...
    return new Promise(async (resolve) => {
      ...
      EventService.emit("popout", request).then(async ({ result }) => {
        ...
        if (KeyPairService.isHardware(account.publicKey)) {
          ...
          signature = await SigningService.sign(
            payload.network,
            payload.transaction,
            account.publicKey,
            true
  ...
  async requestParser(transaction, network, abi, token = null) {
    ...
    if (transaction.to && transaction.to.length) data.to = transaction.to;
